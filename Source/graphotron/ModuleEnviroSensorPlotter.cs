...
namespace graphotron
{
	public class ModuleEnviroSensorPlotter : PartModule
	{
		...
		private string tooltipoff = "Show Graphotron UI";
		private string tooltipon = "Hide Graphotron UI";
		private ApplicationLauncherButton appbutton;
		private const string btextureOff = "Graphotron/Textures/graphotron_icon_off";
		private const string btextureOn = "Graphotron/Textures/graphotron_icon_on";
		//applauncher
		...
		public void TogglePlotterUI()
		{
			...
			if (isWindowShownMain && ToolbarManager.ToolbarAvailable)
			{
				button.TexturePath = btextureOn;
				button.ToolTip = tooltipon;
			}
			else if (!isWindowShownMain && ToolbarManager.ToolbarAvailable)
			{
				button.TexturePath = btextureOff;
				button.ToolTip = tooltipoff;
			}
		}
		...
		public override void OnStart(StartState state)
		{
			...
			{
				button = ToolbarManager.Instance.add("graphotron", "GraphotonGUI");
				button.TexturePath = btextureOff;
				button.ToolTip = tooltipoff;
				button.OnClick += (e) => TogglePlotterUI();
				button.Visibility = new GameScenesVisibility(GameScenes.FLIGHT);
			}
			else
			{
				LoadTexture(ref buttonTexture, "graphotron_icon.png", KSPUtil.ApplicationRootPath.Replace("\\", "/") + "GameData/Graphotron/Textures/");
				appbutton = ApplicationLauncher.Instance.AddModApplication(TogglePlotterUI, TogglePlotterUI, null, null, null,
					null, ApplicationLauncher.AppScenes.FLIGHT, (Texture)buttonTexture);
			}
			...
			try
			{
				GameEvents.onPartExplode.Add(this.CSVExportEx);
				GameEvents.OnVesselRecoveryRequested.Add(this.CSVExportEx);
			}
		...
		{
			if (isPlotting && saveOnCrash && part.name == "graphotron")
			{
				CSVExport("-" + FlightGlobals.ActiveVessel.vesselName + "-Crashed");
			}
		}

		void CSVExportEx(Vessel v)
		{
			if (isPlotting && saveOnCrash)
			{
				CSVExport("-" + FlightGlobals.ActiveVessel.vesselName + "-Recovered");
			}
		}
		...
		private void DrawWindowMain(int windowID)
		{
			...
			if (GUILayout.Button("Save to PNG", GUILayout.Width(halfWidth)))
			{
				...
				KSP.IO.File.WriteAllBytes<ModuleEnviroSensorPlotter>(pbytes, DateTime.Now.ToString("yyyy-MM-dd hhmmss") + "-" + FlightGlobals.ActiveVessel.vesselName + ".png", null);
			}

			if (GUILayout.Button("Save to CSV", GUILayout.Width(halfWidth)))
			{
				this.CSVExport("-" + FlightGlobals.ActiveVessel.vesselName);
			}
			GUILayout.EndHorizontal();
		...
		private void DrawWindowOptions(int WindowID)
		{
			...
			saveOnCrash = GUILayout.Toggle(saveOnCrash, "Export on crash/recovery");
		...
		public static Boolean LoadTexture(ref Texture2D tex, String file, String folder)
		{
			Boolean Loaded = false;
			//File Exists check
			if (System.IO.File.Exists(String.Format("{0}/{1}", folder, file)))
			{
				tex.LoadImage(System.IO.File.ReadAllBytes(String.Format("{0}/{1}", folder, file)));
				Loaded = true;
			}
			return Loaded;
		}
